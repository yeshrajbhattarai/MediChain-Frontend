...
//   POST /api/v1/staff/nurses/         — add new nurse
//   GET  /api/v1/staff/nurses/<uuid>/  — view nurse detail
//   POST /api/v1/staff/nurses/<uuid>/  — toggle active / inactive
...
function AddNurseModal({ open, onClose, onAdded }) {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await api('/staff/nurses/', { method: 'POST', body: JSON.stringify(form) })
      const data = await res.json()
      if (!res.ok) { setErrors(data.errors || { general: 'Something went wrong.' }); return }
      setSuccess(`${data.nurse.full_name} added successfully.`)
      setForm(blank)
      onAdded(data.nurse)
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add New Nurse">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input className={inputCls('full_name')} placeholder="Nurse Priya Nair"
          ...
          <div className="col-span-2">
            ...
            <input type="email" className={inputCls('email')} placeholder="nurse@hospital.com"
          ...
          <div>
            ...
            <input className={inputCls('employee_id')} placeholder="NRS-001"
        ...
        <p className="text-xs text-gray-400">Login credentials will be sent to the nurse's email automatically.</p>
...
function NurseDetailModal({ nurse, open, onClose, onToggled }) {
  ...
  useEffect(() => {
    if (!open || !nurse) return
    if (hasFetched.current) return
    hasFetched.current = true
    setLoading(true)
    api(`/staff/nurses/${nurse.id}/`).then(r => r.json()).then(setDetail).catch(() => {}).finally(() => setLoading(false))
  }, [open, nurse])

  function handleClose() {
    hasFetched.current = false
    setDetail(null); setToggleMsg(''); onClose()
  }

  async function handleToggle() {
    setToggling(true); setToggleMsg('')
    try {
      const res = await api(`/staff/nurses/${nurse.id}/`, { method: 'POST' })
      const data = await res.json()
      if (res.ok) {
  ...
  const d = detail || nurse || {}

  return (
    <Modal open={open} onClose={handleClose} title="Nurse Profile" width="max-w-xl">
      {loading ? <Spinner /> : (
        <div className="space-y-5">
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500">Nurse · {d.hospital_name}</p>
          ...
          {toggleMsg && (
            <p className="text-sm text-center text-blue-600 bg-blue-50 px-3 py-2 rounded-lg">{toggleMsg}</p>
          )}
          ...
          <div className="flex gap-2 pt-1 border-t border-gray-100">
            ...
            <button onClick={handleToggle} disabled={toggling}
              ...
              {toggling ? 'Updating…' : d.status === 'active' ? 'Deactivate Nurse' : 'Activate Nurse'}
            </button>
          </div>
        </div>
      )}
...
export default function Nurses() {
  ...
  async function fetchNurses() {
    setLoading(true)
    try {
      const res = await api('/staff/nurses/')
      const data = await res.json()
  ...
  function handleAdded(nurse) { setNurses(prev => [nurse, ...prev]) }
  function handleToggled(updated) {
    setNurses(prev => prev.map(n => n.id === updated.id ? { ...n, status: updated.status } : n))
  }
  function openDetail(nurse) { setSelected(nurse); setShowDetail(true) }

  const visible = nurses.filter(n => {
    const matchesFilter = filter === 'all' || n.status === filter
    const q = search.toLowerCase()
    const matchesSearch = !q || [n.full_name, n.email, n.employee_id, n.phone]
      .some(v => v?.toLowerCase().includes(q))
    return matchesFilter && matchesSearch
  })

  const totalActive   = nurses.filter(n => n.status === 'active').length
  const totalInactive = nurses.filter(n => n.status === 'inactive').length
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            {nurses.length} total · {totalActive} active · {totalInactive} inactive
          </p>
        </div>
        <button onClick={() => setShowAdd(true)}
          ...
          <span className="text-lg leading-none">+</span> Add Nurse
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? <Spinner /> : visible.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="font-medium text-gray-600">No nurses found</p>
            <p className="text-sm mt-1">
              {search || filter !== 'all' ? 'Try adjusting your search or filters.' : 'Add your first nurse to get started.'}
            </p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Nurse</th>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-medium text-gray-900 truncate">{nurse.full_name}</p>
                        <p className="text-xs text-gray-400 truncate">{nurse.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-gray-500 font-mono text-xs hidden lg:table-cell">{nurse.employee_id}</td>
                  <td className="px-5 py-3.5 text-gray-600 hidden md:table-cell">{nurse.phone}</td>
                  <td className="px-5 py-3.5"><Badge status={nurse.status} /></td>
                  <td className="px-5 py-3.5 text-right">
                    <button onClick={e => { e.stopPropagation(); openDetail(nurse) }}
      ...
      {!loading && visible.length > 0 && (
        <p className="text-xs text-gray-400 text-right">Showing {visible.length} of {nurses.length} nurses</p>
      )}

      <AddNurseModal open={showAdd} onClose={() => setShowAdd(false)} onAdded={handleAdded} />
      <NurseDetailModal
        nurse={selected} open={showDetail}
